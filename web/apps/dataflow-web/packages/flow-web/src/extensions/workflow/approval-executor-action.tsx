  ...
  components: {
    Config: forwardRef(
      (
        ...
        const refs = useMemo(
          () =>
            (parameters?.contents || [])?.map(() => createRef<Validatable>()),
          [parameters],
        );

        useEffect(() => {
          if (platform === "console" && parameters.contents.length === 0) {
        ...
        return (
          ...
          >
            {platform === "client" && (
              ...
              >
                <Input placeholder={t("approval.titlePlaceholder")} />
              </FormItem>
            )}
            <FormItem
            ...
            >
              <AuditFlowKeyConfig
                ...
                drawerClassName={
                  platform === "client" ? styles.AuditFlowDrawer : undefined
                }
                getPopupContainer={getPopupContainer}
                processType={audit_type}
                allowOwnerAuditor={true}
                onlyOwnerAuditor={
                  microWidgetProps?.mode === StrategyMode.perm ||
                  microWidgetProps?.mode === StrategyMode.upload
                }
                platform={platform}
              />
            </FormItem>
                ...
                {(fields, { add, remove, move }, { errors }) => {
                  return (
                    <>
                        ...
                        overlay={
                          <Menu
                            ...
                            onClick={(e) =>
                              add({
                                type: e.key,
                                title: t(getApproveLabel(e.key, isSecretMode)),
                                value: undefined,
                              })
                            }
                          />
                      ...
                      >
                        <Button>{t("approval.addContent")}</Button>
                      </Dropdown>
                    </>
                  );
                }}
                  ...
                  <>
                    <tr>
                      <td style={{ paddingTop: "12px" }}>
                        {t("approval.contents") + " " + String(index + 1)}
                      </td>
                    </tr>
                    <tr>
                      <td className={styles.label}>
                        {t("approval.contentType", "审核内容类型")}
                        {t("colon", "：")}
                      </td>
                      <td>{t(`approval.${item.type}`, "")}</td>
                    </tr>
                    <tr>
                      <td className={styles.label}>
                        {t("approval.contentTitle", "标题")}
                        {t("colon", "：")}
                      </td>
                      <td>{item.title}</td>
                    </tr>
                    {typeof item?.allowModifyByAuditor === "boolean" && (
                      <tr>
                        <td className={styles.label}>
                          {t("approval.allowModifyByAuditor", "允许审核员修改")}
                          {t("colon", "：")}
                        </td>
                        <td>
                          {t(
  ...
  ({ t, value, onChange, onClose, index }, ref) => {
    ...
    const getItemStyle = (isDragging: boolean, draggableStyle: any) => {
      ...
      return {
        userSelect: "none",
        cursor: isDragging ? "move" : "default",
        boxShadow: isDragging ? "0 2px 9px 1px rgba(0, 0, 0, 0.1)" : undefined,
        ...
        {(provided, snapshot) => (
          ...
          >
            ...
            >
              ...
              >
                <Input placeholder={t("approval.contentTitlePlaceholder")} />
              </FormItem>
              <FormItem
                name="value"
                label={
                ...
                {(() => {
                  switch (value?.type) {
                    ...
                    case "asFile":
                      return (
                        <AsFileSelect
                          ...
                          omitUnavailableItem
                          omittedMessage={t("unavailableFilesOmitted")}
                    ...
                    case "string":
                      return (
                        <Input
                          placeholder={t("approval.content.stringPlaceholder")}
                        />
                      );
                    default:
                      return <Input readOnly placeholder={t("selectParams")} />;
                  }
                })()}
              </FormItem>
              <FormItem
                ...
                hidden={!["string", "long_string"].includes(contentType)}
              >
                <Checkbox>{t("approval.allowModifyByAuditor")}</Checkbox>
              </FormItem>
            </Form>
          </div>
        )}
