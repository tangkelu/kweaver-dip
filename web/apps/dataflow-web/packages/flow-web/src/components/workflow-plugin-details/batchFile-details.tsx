...
export function BatchFileDetails({
  ...
  async function getFilePathById(id: string) {
    ...
    for (const lib of libs) {
      if (lib.doc_libs && lib.doc_libs.some((item) => id.startsWith(item.id))) {
        // 涉密 共享文档库名称变更
        if (
          ...
          try {
            const { data } = await API.appStore.apiAppstoreV1SecretConfigGet();
        ...
        if (lib.subtypes) {
          for (const subtype of lib.subtypes) {
            if (
              (subtype.doc_libs as unknown as efast.EntryDoc[]).some((item) =>
      ...
      if (fileArr.length) {
        ...
        if (errorArr[0]?.data.code === 404002006) {
          microWidgetProps?.components?.messageBox({
            ...
            message: t("err.404002006", "当前文档已不存在或其路径发生变更。"),
            okText: t("ok"),
          });
          return;
        }
...
function WorkflowPluginFileDetailsContent({
  ...
  const enablePreview = useMemo(() => {
    if (
      ...
      (audit_status !== WorkflowAuditStatus.Pending &&
        target !== AuditPageTarget.applyPage)
    ) {
      return false;
    }
    return true;
    ...
    async () => {
      if (currentDirId) {
        try {
          ...
          return [...data.dirs, ...data.files].map(({ name, docid, size }) => ({
            name,
            docid,
            isDirectory: size === -1,
            path,
  ...
  const handleBatchDownload = async () => {
    ...
    try {
      ...
      if (package_address) {
        const token = (config.headers?.authorization as string).slice(7);
