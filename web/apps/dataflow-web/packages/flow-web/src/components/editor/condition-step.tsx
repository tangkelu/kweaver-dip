...
import { BranchesOperator, IStep, ParallelOperator } from "./expr";
      ...
      >
        <>
          <div
            ...
            onClick={(e) => {
              if(step?.operator === BranchesOperator) onConfigConditions(step, branchIndex, onChange);
              e.stopPropagation();
            }}
          >
            <div className={styles.head}>
              <div className={styles.title}>
                {(stepNodes[step.branches![branchIndex].id]?.index || 0) + 1}
                .&nbsp;
              ...
              <div className={styles.priority}>
                {step?.operator === ParallelOperator
                  ? t("editor.step.simultaneouslyExecute")
                  : t("editor.step.conditionPriority", "优先级 {index}", {
                      index: branchIndex + 1,
                    })}
              </div>
            ...
            {step?.operator === BranchesOperator && (
              <div className={styles.body}>
                {count === 0 ? (
                  <div className={styles.conditionEmpty}>
                    ...
                    <Button type="link">
                      {t("editor.step.conditionButton", "设置条件")}
                    </Button>
                  </div>
                ) : (
