...
export const SelectAtlas = ({
  ...
  useEffect(() => {
    async function fetchGraphOptions() {
      try {
        const { data: knwData } = await API.axios.get(
  ...
  const handleGraphChange = async (value: { value: string; label: string }) => {
    ...
    try {
      ...
      const defaultEntityIds = DEFAULT_ENTITIES.map((name) => {
        const id = entityNameToId[name];
        if (id) {
          const option = entityOptions.find(
            (opt: ISelectAtlasInfo) => opt.value === id,
          );
      ...
      const defaultEdgeIds = DEFAULT_EDGES.map((name) => {
        const id = edgeNameToId[name];
        if (id) {
          const option = edgeOptions.find(
            (opt: ISelectAtlasInfo) => opt.value === id,
          );
          return option ? { value: id, label: option.label } : null;
        }
  ...
  const handleChangeEntities = (value: ISelectAtlasInfo[]) => {
    ...
    const selectEntityNames = atlas.entity
      .filter((v) => entityKey.includes(v.entity_id))
      .map((v) => v.name);

    const selectEdge = atlas.edge
      .filter(
        (v) =>
    ...
    [ItemType.IndexBase]: {
      Component: (
        <div>
          ...
          >
            {t(
              "datastudio.create.pdfParse.parseResultStorage",
              "解析结果存储至",
            )}
          </div>
          <div
          ...
          >
            <FormItem
              required
              label={t(
                "datastudio.create.pdfParse.documentMetadataIndex",
                "文档元数据索引库",
              )}
            >
              <Select
                ...
                disabled
                options={[
                  { label: "content_document", value: "content_document" },
                ]}
              />
            </FormItem>
            <FormItem
              required
              label={t(
                "datastudio.create.pdfParse.documentElementIndex",
                "文档元素索引库",
              )}
            >
              <Select
                ...
                disabled
                options={[
                  { label: "content_element", value: "content_element" },
                ]}
              />
            </FormItem>
            <FormItem
    ...
    <Form form={form} layout="vertical" initialValues={initialValues}>
      {selectTypes?.map((type) => {
        if (type in formItems) {
          ...
          if (ItemProps) {
            return <FormItem {...ItemProps}>{Component}</FormItem>;
          } else {
            return Component;
          }
        } else {
