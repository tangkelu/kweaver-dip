...
export const MicroAppProvider: FC<{
  ...
  let prefix =
    ...
    microWidgetProps?.config?.systemInfo?.as_access_prefix;
  if (!prefix || prefix === "/") {
    prefix = "";
  }
  ...
  useMount(() => {
    ...
    API.setup({
      ...
      getToken: () =>
        microWidgetProps?.token?.getToken?.access_token ||
        cookies.get("studio.oauth2_token"),
  ...
  const [navigation, setNavigation] = useState<Record<string, NavigationItem>>({
    documents: {
      ...
      if (data?.length) {
        data.forEach((item) => {
          if (item.page_config === page_config) {
            ...
            const childRes = getTargetNavigation(page_config, item.nodes);
            if (childRes) {
              res = childRes;
            }
          }
  ...
  useEffect(() => {
    ...
    async function getSecretConfig() {
      try {
        ...
        } = await API.axios.get(`${prefixUrl}/api/appstore/v1/secret-config`);
      ...
      >
          ...
          onError={(e) => {
            if (
              ...
              const missingMessages =
                ...
                ((Error as any)[`__${APP_NAME}_MISSING_MESSAGES`] = {});
              const messages =
                missingMessages[locale] || (missingMessages[locale] = {});
        ...
        >
          ...
          >
            <>
              {children}
              <Fragment key="modal">{modalContextHolder}</Fragment>
              <Fragment key="message">{messageContextHolder}</Fragment>
