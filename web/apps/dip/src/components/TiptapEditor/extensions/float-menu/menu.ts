...
export interface FloatMenuItemStorage {
  floatMenu?: {
    ...
    items?:
      | FloatMenuButtonItem
      | FloatMenuRenderItem
      | Array<FloatMenuButtonItem | FloatMenuRenderItem>
  }
}
...
export const FloatMenu = Extension.create<FloatMenuOptions>({
  ...
  addProseMirrorPlugins() {
    ...
    for (const [name, storage] of Object.entries(
      this.editor.storage as unknown as Record<string, FloatMenuItemStorage>,
    )) {
      if (storage?.floatMenu) {
        ...
        if (storage.floatMenu.items) {
          const menus = Array.isArray(storage.floatMenu.items)
    ...
    return [
      new Plugin({
        ...
        view: FloatMenuView.create({
          ...
          show: ({ editor }) => {
            const { state, isEditable, view } = editor
            if (!isEditable) {
              return false
            }
            // 如果正在拖动，不显示菜单
