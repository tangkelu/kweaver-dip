...
export interface FloatMenuInputViewOptions {
  ...
  onBlur?: (value: string, root: HTMLInputElement, event: FocusEvent) => void
  onKey?: (
...
export class FloatMenuView implements PluginView {
  ...
  public update(view: EditorView, prevState: EditorState) {
    ...
    if (
      view.composing ||
      (prevState.doc.eq(state.doc) && prevState.selection.eq(state.selection))
    ) {
      return
    }
  ...
  public createInput(options: FloatMenuInputViewOptions) {
    ...
    root.classList.add('dip-prose-mirror-fm-input')
    ...
    if (options.onBlur) {
      input.addEventListener('blur', (e) => {
        if (options.onBlur) {
          options.onBlur(input.value, input, e)
        }
      })
    }
    if (options.onKey) {
  ...
  public createTextarea(options: FloatMenuTextareaViewOptions) {
    ...
    root.classList.add('dip-prose-mirror-fm-textarea')
    for (const clazz of options.classes ?? []) {
      root.classList.add(clazz)
    }
    for (const [key, val] of Object.entries(options.attributes ?? {})) {
  ...
  public createButton(options: FloatMenuButtonViewOptions) {
    ...
    root.classList.add('dip-prose-mirror-fm-button')
    if (options.onClick) {
    ...
    if (options.shortcut) {
      ...
      shortcut.classList.add('dip-prose-mirror-fm-button-shortcut')
  ...
  public createForm() {
    const root = document.createElement('div')
    root.classList.add('dip-prose-mirror-fm-form')
    return root
  }

  public createAction() {
    const root = document.createElement('div')
    root.classList.add('dip-prose-mirror-fm-action')
    return root
  }

  public createDivider() {
    const root = document.createElement('span')
    root.classList.add('dip-prose-mirror-fm-divider')
    return root
  }
  ...
  private _element() {
    ...
    element.classList.add('dip-prose-mirror-fm')
