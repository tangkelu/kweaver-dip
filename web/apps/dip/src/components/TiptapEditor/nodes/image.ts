...
import { Image as TImage, type ImageOptions as TImageOptions } from '@tiptap/extension-image'
...
import type { UploaderItemStorage, UploaderStorage } from '../extensions/uploader'
...
export const Image = TImage.extend<ImageOptions>({
  name: 'image',
  addOptions() {
    const parentOptions = this.parent?.() ?? ({} as TImageOptions)
    return {
      ...parentOptions,
  ...
  addStorage() {
    return {
      ...
      markdown: {
        ...
        hooks: {
          afterParse: (root) =>
            this.options.inline ? root : unwrap(root, (node) => node.type === 'image'),
          beforeSerialize: (root) =>
            this.options.inline ? root : wrap(root, (node) => node.type === 'image'),
        },
      },
      uploader: {
        ...
        view: FloatMenuView.create({
          ...
          onInit: ({ view, editor, root }) => {
            ...
            const upload = view.createUpload({
              ...
              accept: 'image/*',
              onUpload: (element) => {
                const uploader = (this.editor.storage as { uploader?: UploaderStorage }).uploader
                if (element.files && uploader) {
                  uploader.upload(element.files).then((items) =>
                    items.forEach((item) => {
