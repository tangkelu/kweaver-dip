    ...
    if (data) {
      ...
      for (let i = 0; i < inputValueArr.length; i++) {
        ...
        for (let j = 0; j < vars.length; j++) {
          ...
          lineStr =
            lineStr.slice(0, varIndex) + ' '.repeat(variable.length) + lineStr.slice(lastIndex)
        }
  ...
  const createVarNode = (
    ...
    return new Promise((resolve) => {
      ...
      if (isVar) {
        ...
        options =
          trigger?.filter((item) => item.character !== '$')?.map((item) => item.options) || []
            ...
            onKeyDown={(e) => {
              if (e.keyCode === 8) {
                if (!getSelectSearchText()) {
                  ...
                  range.setEnd(
                    focusNode,
                    currentTriggerCharacterIndex.current + 1 + searchStrRef.current.length,
                  )
                  range.collapse()
                  inputRef.current?.focus()
                }
