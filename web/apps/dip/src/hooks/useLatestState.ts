...
type LatestStateFunc = <S>(
  initialState: S,
) => [S, Dispatch<SetStateAction<S>>, GetStateAction<S>, ResetStateAction]
...
const useLatestState: LatestStateFunc = (initialState) => {
  ...
  const setState = useCallback((currentState: any) => {
    if (!unMountedRef.current) {
      const stateData =
        typeof currentState === 'function' ? currentState(latestStateRef.current) : currentState
