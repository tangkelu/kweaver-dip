    ...
    <div>
      <div className={styles['table-operation-bar']}>
        ...
        <div className='g-flex-center'>
          ...
          {hasRefreshButton && <Button.Icon title='刷新' icon={<IconFont type='icon-dip-refresh' className='g-rotate-h' />} onClick={() => onRefresh()} />}
        </div>
      </div>
      {showFilter &&
        moreFilter && ( // 有多个筛选条件的时候
          <div className={styles['table-operation-filter-bar']}>
            ...
            <Button.Icon className='g-ml-2 g-mr-2' icon={<IconFont type='icon-dip-close' />} onClick={() => setShowFilter(false)} />
          </div>
        )}
    </div>
  );
