...
const ColumnsController = (props: any) => {
  ...
  const items: MenuProps['items'] = [
    { key: 'reset', label: intl.get('components.table.controller.resetColumnWidth') },
    { key: 'adapter', label: intl.get('components.table.controller.suitableAllColumnWidths') },
    { type: 'divider' },
    {
      key: 'all',
      label: (
        <Checkbox indeterminate={indeterminate} checked={allChecked}>
          {intl.get('components.table.controller.selectAll')}
        </Checkbox>
      ),
    },
