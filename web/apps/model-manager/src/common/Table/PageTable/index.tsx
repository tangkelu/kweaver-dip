...
const PageTable: React.FC<CustomTableProps> = props => {
  ...
  const initSessionColumnsController = (columns: any[]) => {
    ...
    if (!temp.checkbox && _rowSelection) {
      temp.checkbox = { dataIndex: 'checkbox', index: -1, title: intl.get('components.table.controller.checkBox'), checked: true, disabled: true };
    }
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      entries => {
        _.forEach(entries, entry => !entry.isIntersecting && setMenuVisible(false));
      },
        ...
        pagination={
          pagination === false
            ...
            : {
                ...
                showTotal: total => intl.get('components.table.total', { total }),
                ...pagination,
              }
        }
        {...otherProps}
