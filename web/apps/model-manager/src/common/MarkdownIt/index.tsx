...
const MarkdownIt = (props: any) => {
  ...
  const addEvent = useCallback(
    _.debounce(() => {
      const copyDoms = document.querySelectorAll('.code-header-copy');
      _.forEach(copyDoms, item => {
        const code = item.getAttribute('__code');
  ...
  const code_string = (language: any, code: any, source: any) => {
    return (
      ...
      language +
      '</span><div class="code-header-copy"' +
      `__code=\`${encodeURIComponent(source)}\`` +
      '>' +
