...
const BasicInformation = (props: CreateAndEditModalProps) => {
  ...
  const onToView = () => {
    form.resetFields(['model_name', 'max_model_len', 'model_parameters']);
    setModalType('view');
  };

  const constructValues = (_values: any) => {
    const values = _.mapValues(_.cloneDeep(_values), value => (typeof value !== 'string' ? value : value.trim()));
    const { model_name, model_series, max_model_len, model_parameters, model_type, api_model, api_url, api_key, secret_key } = values;
    ...
    const result: any = { model_config, model_name, model_series, max_model_len, model_type, model_id: sourceData.model_id };
