...
import { Input, Button, IconFont, Select } from '@/common';
...
const Operation: React.FC<OperationProps> = props => {
  const { children, nameConfig, sortConfig, initialFilter, onChange, onRefresh, showFilter: _showFilter = true } = props;
  const { key: ncKey, value: ncName = '', placeholder: ncPlaceholder = '请输入名称' } = nameConfig || {}; // 这样是为了注入时避免key的二次传递
    ...
    <div>
      <div className={styles['table-operation-bar']}>
        ...
        <div className='g-flex-center'>
          {(oneFilter || noFilter) && ( // 只有一个筛选条件或者没有筛选条件的时候
            <React.Fragment>
              <Input.Search allowClear value={ncName} style={{ width: 250, marginRight: noFilter ? 12 : 0 }} placeholder={ncPlaceholder} onChange={onSearch} />
              {!noFilter && <div className='g-ml-3 g-mr-3'>{filterElement}</div>}
            </React.Fragment>
          )}
          {moreFilter && ( // 有多个筛选条件的时候
            <Button.Icon
              title='展开'
