import styles from './index.module.less';
import { Splitter } from 'antd';
import { CloseOutlined } from '@ant-design/icons';
import DipButton from '@/components/DipButton';
import React from 'react';
import DipIcon from '@/components/DipIcon';
import { useDipChatStore } from '@/components/DipChat/store';
import { AdMonacoEditor } from '@/components/Editor/AdMonacoEditor';
import { DipChatItemContentProgressType } from '@/components/DipChat/interface';
import ScrollBarContainer from '@/components/ScrollBarContainer';
import Markdown from '@/components/Markdown';
import { removeInvalidCodeBlocks } from '@/components/Markdown/utils';
import { FileTypeIcon, getFileExtension } from '@/utils/doc';

const CodeToolSide = () => {
  const {
    dipChatStore: { activeChatItemIndex, chatList, activeProgressIndex },
    closeSideBar,
  } = useDipChatStore();
  const chatItem = chatList[activeChatItemIndex];
  const activeProgress: DipChatItemContentProgressType = chatItem.content.progress[activeProgressIndex] || {};

  const getInput = () => {
    const inputStr = activeProgress.codeResult?.input;
    if (inputStr) {
      return inputStr;
    }
    return JSON.stringify(activeProgress.originalAnswer || '', null, 2);
  };

  const renderOutput = () => {
    const outputStr = activeProgress.codeResult?.actionResult?.stdout;
    const renderContent = () => {
      const action = activeProgress.codeResult?.action;
      const inputStr = activeProgress.codeResult?.input;
      if (action === 'execute_command' && inputStr?.includes('ls')) {
        // 说明是ls命令，那么就显示文件列表
        const fileName = outputStr?.split('\n') || [];
        if (fileName.length > 0) {
          return fileName.map((file: string, index: number) => {
            if (!file.includes('.')) {
              return file;
            }
            return (
              <div className="dip-flex-align-center dip-w-100" key={index}>
                <FileTypeIcon extension={getFileExtension(file)} fontSize={16} />
                <div className="dip-flex-item-full-width dip-ellipsis dip-ml-8">{file}</div>
              </div>
            );
          });
        }
      }
      return <Markdown readOnly value={removeInvalidCodeBlocks(outputStr, true)} />;
    };
    if (outputStr) {
      return (
        <Splitter.Panel>
          <div className="dip-p-12 dip-full dip-flex-column dip-overflow-hidden">
            <div className="dip-mb-16">输出</div>
            <div className="dip-flex-item-full-height">
              <ScrollBarContainer className="dip-full">{renderContent()}</ScrollBarContainer>
            </div>
          </div>
        </Splitter.Panel>
      );
    }
    return null;
  };

  return (
    <Splitter className={styles.container} layout="vertical">
      <Splitter.Panel>
        <div className="dip-flex-column dip-p-12 dip-full dip-overflow-hidden">
          <div className="dip-flex-space-between">
            <div className="dip-flex-item-full-width dip-flex-align-center">
              <DipIcon className="dip-font-16" type="icon-dip-color-python" />
              <div title={activeProgress.title} className="dip-ml-8 dip-flex-item-full-width dip-ellipsis">
                {activeProgress.title}
              </div>
            </div>
            <DipButton size="small" type="text" onClick={closeSideBar}>
              <CloseOutlined className="dip-text-color-45 dip-font-16" />
            </DipButton>
          </div>
          <div className="dip-flex-item-full-height dip-mt-12">
            <AdMonacoEditor
              className={styles.editor}
              value={getInput()}
              options={{
                minimap: { enabled: false },
                fontSize: 14,
                tabSize: 2,
                insertSpaces: true,
                readOnly: true,
                scrollbar: {
                  alwaysConsumeMouseWheel: false, // 禁用Monaco的默认滚轮事件
                },
                lineNumbersMinChars: 4,
                unicodeHighlight: {
                  ambiguousCharacters: false, // 关闭中文符号高亮报警
                },
                scrollBeyondLastLine: false, // 禁止滚动超出最后一行
                wordWrap: 'on', // 自动换行，文本始终适应编辑器宽度
                automaticLayout: true, // 自动布局
              }}
              defaultLanguage="python"
            />
          </div>
        </div>
      </Splitter.Panel>
      {renderOutput()}
    </Splitter>
  );
};

export default CodeToolSide;
