...
const SqlToolSide = () => {
  ...
  const getSql = () => {
    const sqlStr = activeProgress.sqlResult?.sql;
    if (sqlStr) {
      return format(sqlStr, { language: 'sql' });
    }
        ...
        <div className="dip-flex-column dip-p-12 dip-full dip-overflow-hidden">
          <div className="dip-flex-space-between">
            <div className="dip-flex-item-full-width dip-flex-align-center">
              <DipIcon className="dip-font-16" type="icon-dip-color-sql" />
        ...
        <div className="dip-p-12 dip-full dip-flex-column dip-overflow-hidden">
          ...
          <div className="dip-flex-item-full-height">
            <ADTable
              autoScrollY
              showHeader={false}
              columns={activeProgress.sqlResult?.tableColumns}
              dataSource={activeProgress.sqlResult?.tableData}
            />
          </div>
        </div>
      </Splitter.Panel>
