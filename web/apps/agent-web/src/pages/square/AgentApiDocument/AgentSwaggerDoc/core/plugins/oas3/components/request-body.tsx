...
import { getCommonExtensions, getSampleSchema, stringify, isEmptyValue } from '../../../utils';
...
export const getDefaultRequestBodyValue = (requestBody, mediaType, activeExamplesKey) => {
  ...
  const mediaTypeExample = hasExamplesKey
    ? mediaTypeValue.getIn(['examples', activeExamplesKey, 'value'])
    : exampleSchema;
...
const RequestBody = ({
  ...
  const setIsIncludedOptions = key => {
    ...
    const currentInclusion = requestBodyInclusionSetting.get(key, 'no value');
  ...
  const sampleForMediaType = rawExamplesOfMediaType?.map((container, key) => {
    ...
    if (val) {
      container = container.set('value', getDefaultRequestBodyValue(requestBody, contentType, key), val);
    }
    return container;
  });
  ...
  if (
    isObjectContent &&
    (contentType === 'application/x-www-form-urlencoded' || contentType.indexOf('multipart/') === 0) &&
    ...
    return (
      <div className="table-container">
        {requestBodyDescription && <Markdown source={requestBodyDescription} />}
        <table>
          <tbody>
            {Map.isMap(bodyProperties) &&
              bodyProperties.entrySeq().map(([key, prop]) => {
                if (prop.get('readOnly')) return;

                const commonExt = showCommonExtensions ? getCommonExtensions(prop) : null;
                ...
                const useInitialValFromSchemaSamples =
                  prop.has('default') ||
                  prop.has('example') ||
                ...
                return (
                  <tr key={key} className="parameters" data-property-name={key}>
                    <td className="parameters-col_name">
                      ...
                      <div className="parameter__type">
                        ...
                        {!showCommonExtensions || !commonExt.size
                          ...
                          : commonExt
                              .entrySeq()
                              .map(([key, v]) => <ParameterExt key={`${key}-${v}`} xKey={key} xVal={v} />)}
                      </div>
                      <div className="parameter__deprecated">{prop.get('deprecated') ? 'deprecated' : null}</div>
                    </td>
                        ...
                        <div>
                          ...
                          {required ? null : (
                            <ParameterIncludeEmpty
                              ...
                              isDisabled={
                                Array.isArray(currentValue) ? currentValue.length !== 0 : !isEmptyValue(currentValue)
                              }
                            />
                          )}
                        </div>
  ...
  const sampleRequestBody = getDefaultRequestBodyValue(requestBody, contentType, activeExamplesKey);
  let language = null;
  const testValueForJson = getKnownSyntaxHighlighterLanguage(sampleRequestBody);
  if (testValueForJson) {
    language = 'json';
  }
