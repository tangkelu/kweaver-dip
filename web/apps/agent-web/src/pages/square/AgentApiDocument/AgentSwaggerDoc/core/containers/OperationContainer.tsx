...
export default class OperationContainer extends PureComponent {
  ...
  mapStateToProps(nextState, props) {
    ...
    const { docExpansion, deepLinking, displayOperationId, displayRequestDuration, supportedSubmitMethods } =
      getConfigs();
    const showSummary = layoutSelectors.showSummary();
    const operationId =
      op.getIn(['operation', '__originalOperationId']) ||
      op.getIn(['operation', 'operationId']) ||
      opId(op.get('operation'), props.path, props.method) ||
      op.get('id');
    const isShownKey = ['operations', props.tag, operationId];
    const isDeepLinkingEnabled = deepLinking && deepLinking !== 'false';
    const allowTryItOut =
      supportedSubmitMethods.indexOf(props.method) >= 0 &&
      (typeof props.allowTryItOut === 'undefined'
        ? props.specSelectors.allowTryItOutFor(props.path, props.method)
        : props.allowTryItOut);
    const security = op.getIn(['operation', 'security']) || props.specSelectors.security();

    // console.log('props.specSelectors', props.specSelectors.responseFor(props.path, props.method));
    // console.log('props.specSelectors',props.path, props.method);
  ...
  toggleShown = () => {
    const { layoutActions, tag, operationId, isShown } = this.props;
    const resolvedSubtree = this.getResolvedSubtree();
    if (!isShown && resolvedSubtree === undefined) {
  ...
  getResolvedSubtree = () => {
    const { specSelectors, path, method, specPath } = this.props;
  ...
  requestResolvedSubtree = () => {
    const { specActions, path, method, specPath } = this.props;
