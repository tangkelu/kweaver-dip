...
import { defaultStatusCode, getAcceptControllingResponse } from '../utils';
...
export default class Responses extends React.Component {
  ...
  render() {
    ...
    const defaultCode = defaultStatusCode(responses);
    ...
    const produces =
      this.props.produces && this.props.produces.size ? this.props.produces : Responses.defaultProps.produces;

    const isSpecOAS3 = specSelectors.isOAS3();

    const acceptControllingResponse = isSpecOAS3 ? getAcceptControllingResponse(responses) : null;
        ...
        <div className="responses-inner">
          {!tryItOutResponse ? null : (
            <div>
              ...
              />
              <h4 className="server-h4">Responses</h4>
            </div>
          )}
          ...
          <table aria-live="polite" className="responses-table" id={regionId} role="region">
            ...
            <tbody>
              {responses
                ...
                .map(([code, response]) => {
                  const className =
                    tryItOutResponse && tryItOutResponse.get('status') === code ? 'response_current' : '';
                  return (
                    <Response
                      key={code}
                      path={path}
