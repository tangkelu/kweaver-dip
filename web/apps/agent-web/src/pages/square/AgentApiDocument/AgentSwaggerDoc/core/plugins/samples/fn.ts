...
import { objectify, isFunc, normalizeArray, deeplyStripKey } from '../../utils';
...
const sanitizeRef = value => deeplyStripKey(value, '$$ref', val => typeof val === 'string' && val.indexOf('#') > -1);

const objectContracts = ['maxProperties', 'minProperties'];
const arrayContracts = ['minItems', 'maxItems'];
const numberContracts = ['minimum', 'maximum', 'exclusiveMinimum', 'exclusiveMaximum'];
...
const liftSampleHelper = (oldSchema, target, config = {}) => {
  ...
  if (oldSchema.properties) {
    ...
    const props = objectify(oldSchema.properties);
...
export const sampleFromSchemaGeneric = (schema, config = {}, exampleOverride = undefined, respectXML = false) => {
  ...
  if (!usePlainValue && (hasOneOf || hasAnyOf)) {
    ...
    if (schema.example !== undefined && schemaToAdd.example !== undefined) {
      ...
      const props = objectify(schemaToAdd.properties);
      for (const propName in props) {
        if (!Object.prototype.hasOwnProperty.call(props, propName)) {
          continue;
        }
  ...
  const _attr = {};
  // 上次SWAGGER-UI改动过，给type加了默认的类型
  const { xml, type = 'array', example, properties, additionalProperties, items } = schema || {};
  ...
  const handleMinMaxItems = sampleArray => {
    if (schema?.maxItems !== null && schema?.maxItems !== undefined) {
      const sampleArray = sampleArray.slice(0, schema?.maxItems);
    }
  ...
  const requiredPropertiesToAdd = () => {
    ...
    let addedCount = 0;
    if (respectXML) {
      schema.required.forEach(key => (addedCount += res[key] === undefined ? 0 : 1));
    } else {
      schema.required.forEach(
        key => (addedCount += res[displayName]?.find(x => x[key] !== undefined) === undefined ? 0 : 1)
      );
    }
    ...
    addPropertyToResult = (propName, overrideE = undefined) => {
      if (schema && props[propName]) {
        ...
        if (props[propName].xml.attribute) {
          const enumAttrVal = Array.isArray(props[propName].enum) ? props[propName].enum[0] : undefined;
      ...
      const t = sampleFromSchemaGeneric((schema && props[propName]) || undefined, config, overrideE, respectXML);
  ...
  if (usePlainValue) {
    ...
    if (type === 'array') {
      ...
      let itemSamples = sample.map(s => sampleFromSchemaGeneric(itemSchema, config, s, respectXML));
    ...
    res[displayName] = !isEmpty(_attr) ? [{ _attr }, sample] : sample;
    return res;
  }
  ...
  if (type === 'array') {
    ...
    if (Array.isArray(items.anyOf)) {
      sampleArray = items.anyOf.map(i =>
        sampleFromSchemaGeneric(liftSampleHelper(items, i, config), config, undefined, respectXML)
      );
  ...
  if (respectXML) {
    res[displayName] = !isEmpty(_attr) ? [{ _attr }, value] : value;
    return res;
  }
...
export const sampleFromSchema = (schema, config, o) => sampleFromSchemaGeneric(schema, config, o, false);
