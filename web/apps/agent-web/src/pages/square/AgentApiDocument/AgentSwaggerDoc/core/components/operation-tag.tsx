...
import { createDeepLinkPath, escapeDeepLinkPath, sanitizeUrl, isFunc } from '../utils';
...
export default class OperationTag extends React.Component {
  ...
  render() {
    const { tagObj, tag, children, oas3Selectors, layoutSelectors, layoutActions, getConfigs, getComponent, specUrl } =
      this.props;
    ...
    return (
      <div className={showTag ? 'opblock-tag-section is-open' : 'opblock-tag-section'}>
        <h3
          ...
          style={{ display: 'none' }}
        >
          <DeepLink enabled={isDeepLinkingEnabled} isShown={showTag} path={createDeepLinkPath(tag)} text={tag} />
          {!tagDescription ? (
            <small></small>
          ) : (
            <small>
          ...
          {!tagExternalDocsUrl ? null : (
            <div className="info__externaldocs">
              <small>
                <Link href={sanitizeUrl(tagExternalDocsUrl)} onClick={e => e.stopPropagation()} target="_blank">
