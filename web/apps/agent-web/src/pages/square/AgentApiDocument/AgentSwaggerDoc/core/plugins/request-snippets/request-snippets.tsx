...
const RequestSnippets = ({ request, requestSnippetsSelectors, getConfigs }) => {
  ...
  const [activeLanguage, setActiveLanguage] = useState(
    requestSnippetsSelectors.getSnippetGenerators()?.keySeq().first()
  );
  ...
  useEffect(() => {
    const childNodes = Array.from(rootRef.current.childNodes).filter(
      node => !!node.nodeType && node.classList?.contains('curl-command')
    );
  ...
  return (
    <div className="request-snippets" ref={rootRef}>
      ...
      >
        <h4 onClick={() => handleSetIsExpanded()} style={{ cursor: 'pointer' }}>
          Snippets
        </h4>
        <button
        ...
        >
          <svg className="arrow" width="10" height="10">
            <use
              href={isExpanded ? '#large-arrow-down' : '#large-arrow'}
              xlinkHref={isExpanded ? '#large-arrow-down' : '#large-arrow'}
            />
          </svg>
        </button>
      </div>
        ...
        <div className="curl-command">
          <div style={{ paddingLeft: '15px', paddingRight: '10px', width: '100%', display: 'flex' }}>
            {snippetGenerators.entrySeq().map(([key, gen]) => {
              return (
                <div style={handleGetBtnStyle(key)} className="btn" key={key} onClick={() => handleGenChange(key)}>
