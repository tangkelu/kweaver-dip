...
import { stringify, getSampleSchema } from '../utils';
...
export class JsonSchemaForm extends Component {
  ...
  render() {
    const { schema, errors, value, onChange, getComponent, fn, disabled } = this.props;
...
export class JsonSchema_array extends PureComponent {
  ...
  UNSAFE_componentWillReceiveProps(props) {
    ...
    if (props.schema !== this.state.schema) this.setState({ schema: props.schema });
  }
  ...
  addItem = () => {
    const newValue = valueOrEmptyList(this.state.value);
    this.setState(
      () => ({
        value: newValue.push(
          getSampleSchema(this.state.schema.get('items'), false, {
  ...
  render() {
    ...
    const needsRemoveError = errors.filter(e => e.needRemove !== undefined).map(e => e.error);
    const value = this.state.value; // expect Im List
    const shouldRenderValue = value && value.count && value.count() > 0 ? true : false;
    ...
    const isArrayItemFile =
      schemaItemsType === 'file' || (schemaItemsType === 'string' && schemaItemsFormat === 'binary') ? true : false;
    ...
    return (
      <div className="json-schema-array">
        {shouldRenderValue
          ? value.map((item, i) => {
              const itemErrors = fromJS([...errors.filter(err => err.index === i).map(e => e.error)]);
              return (
                  ...
                  {!disabled ? (
                    <Button
                      className={`btn btn-sm json-schema-form-item-remove ${
                        needsRemoveError.length ? 'invalid' : null
                      }`}
...
export class JsonSchemaArrayItemFile extends Component {
  ...
  render() {
    const { getComponent, errors, disabled } = this.props;
    const Input = getComponent('Input');
...
export class JsonSchema_boolean extends Component {
  ...
  render() {
    ...
    errors = errors.toJS ? errors.toJS() : [];
    const enumValue = schema && schema.get ? schema.get('enum') : null;
    const allowEmptyValue = !enumValue || !required;
    const booleanValue = !enumValue && ['true', 'false'];
    const Select = getComponent('Select');
...
const stringifyObjectErrors = errors => {
  return errors.map(err => {
    ...
    const stringError = typeof err === 'string' ? err : typeof err.error === 'string' ? err.error : null;
...
export class JsonSchema_object extends PureComponent {
  ...
  render() {
    let { getComponent, value, errors, disabled } = this.props;
