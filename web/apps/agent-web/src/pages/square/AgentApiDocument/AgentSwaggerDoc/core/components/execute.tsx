...
export default class Execute extends Component {
  ...
  handleValidateRequestBody = () => {
    const { path, method, specSelectors, oas3Selectors, oas3Actions } = this.props;
    ...
    const oas3RequiredRequestBodyContentType = specSelectors.getOAS3RequiredRequestBodyContentType([path, method]);
    const oas3RequestBodyValue = oas3Selectors.requestBodyValue(path, method);
    if (isJSONString(oas3RequestBodyValue)) {
      const requestBodyValue = JSON.parse(oas3RequestBodyValue);
      if (requestBodyValue.stream === true) {
        message.info('当前页面不支持流式请求调试，请修改stream为false，再进行调试');
        return false;
      }
    }
  ...
  handleValidationResultPass = () => {
    const { specActions, operation, path, method } = this.props;
  ...
  render() {
    ...
    return (
      <button className="btn execute opblock-control__btn" onClick={this.onClick} disabled={disabled}>
        {intl.get('dataAgent.apiDocument.run')}
      </button>
    );
  }
}
